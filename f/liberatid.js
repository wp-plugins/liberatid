...
jQuery(function() {
	...
	jQuery('#liberatid_status_link').click( function() {
		jQuery('#liberatid_system_status').toggle();
		return false;
	});
});
...
function stylize_profilelink_liberatid() {
	jQuery("#commentform a[href$='profile.php']").addClass('liberatid_comment_icon');
}
 ...
 */
function add_liberatid_to_comment_form(wp_url, nonce) {
	var openid_nonce = nonce;

	var openid_comment = jQuery('#liberatid_comment');
	var openid_checkbox = jQuery('#login_with_liberatid');
	var url = jQuery('#url');
	...
	function check_openid( url ) {
		...
		jQuery.getJSON(wp_url + '?liberatid=ajax', {url: url.val(), _wpnonce: openid_nonce}, function(data, textStatus) {
