<?php
/*
 Plugin Name: LiberatID
 Plugin URI: http://wordpress.org/extend/plugins/liberatid
 Description: Allows the use of LiberatID (or OpenID) for account registration, authentication, and commenting.  Also includes an OpenID provider which can turn WordPress author URLs into OpenIDs.
 Author: LiberatID, Inc.
 Author URI: http://liberatid.com/
 Version: 1.0.1
 ...
 Text Domain: liberatid
 */

define ( 'LIBERATID_PLUGIN_REVISION', preg_replace( '/\$Rev: (.+) \$/', '\\1',
...
define ( 'LIBERATID_DB_REVISION', 24426);


$liberatid_include_path = dirname(__FILE__);
...
register_activation_hook('liberatid/liberatid.php', 'liberatid_activate_plugin');
register_deactivation_hook('liberatid/liberatid.php', 'liberatid_deactivate_plugin');
register_uninstall_hook('liberatid/liberatid.php', 'liberatid_uninstall_plugin');

// run activation function if new revision of plugin
if ( get_option('liberatid_plugin_revision') === false || LIBERATID_PLUGIN_REVISION != get_option('liberatid_plugin_revision') ) {
	add_action('admin_init', 'liberatid_activate_plugin');
}
 ...
 * @since 1.0
 */
function is_user_liberatid($user = null) {
    if (is_null($userOpenIDURLs)) {
        $userOpenIDURLs = get_user_openids($user);
    }
    ...
    if ( empty($userOpenIDURLs) ) {
        ...
        foreach ($userOpenIDURLs as &$value) {
            if (stripos($value, $liberatIDString)) {
                return TRUE;
            }
        }
        return FALSE;
 ...
 * <pre><?php echo ( is_comment_liberatid() ? 'Submitted with OpenID' : '' ); ?></pre>
 *
 ...
 * @since 1.0
 */
function is_comment_liberatid($id = null) {
	if ( is_numeric($id) ) {
	...
	if ( is_array($liberatid_comments) ) {
		if ( in_array($comment->comment_ID, $liberatid_comments) ) {
			return true;
		}
	}
 ...
 *
 * @param string $liberatid identifier to match
 ...
 * a symlink).  The standard value should be 'liberatid/liberatid.php' unless files or folders have
 * been renamed.
 *
 * @return string plugin file
 */
function liberatid_plugin_file() {
	static $file;
